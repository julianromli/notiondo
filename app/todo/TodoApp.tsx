...
import { createDraftTask, type OpenCell, type Task, type TodoAppUser } from './types';
import { TaskTableRow } from './TaskTableRow';
import { TaskModal } from './TaskModal';
import type { TodoDataPayload } from './data';
import {
  addPriorityOptionAction,
  addProjectOptionAction,
  addStatusOptionAction,
  createTaskAction,
  deleteTaskAction,
  reorderTasksAction,
  updateTaskAction,
...
const TASK_GROUP_ORDER: Task['group'][] = ['Today', 'This Week', 'Later'];

const DROP_ID: Record<Task['group'], string> = {
  Today: 'drop-today',
  'This Week': 'drop-this-week',
  Later: 'drop-later',
};

function groupFromDropId(id: string): Task['group'] | null {
  for (const g of TASK_GROUP_ORDER) {
    if (DROP_ID[g] === id) return g;
  }
  return null;
}

function idsInGroup(ts: Task[], group: Task['group']): string[] {
  return ts
...
function rebuildTasksFromGroups(prev: Task[], byGroup: Record<Task['group'], string[]>): Task[] {
  const m = new Map(prev.map((t) => [t.id, t]));
  const out: Task[] = [];
  for (const g of TASK_GROUP_ORDER) {
    byGroup[g].forEach((id, idx) => {
      const t = m.get(id);
      if (t) out.push({ ...t, group: g, sortIndex: idx });
    });
  }
  return out;
}

function GroupEmptyDropTarget({ id }: { id: string }) {
  const { setNodeRef, isOver } = useDroppable({ id });
  return (
    <div
      ref={setNodeRef}
      className={`flex items-center text-sm py-3 border-b border-dashed border-[#ececeb] text-[#a5a4a2] justify-center min-h-[44px] ${
        isOver ? 'bg-[#e8f4fd] text-[#187abf]' : ''
      }`}
    >
...
/** Prefer pointer hit target for stacked vertical lists; fallback limits wrong-section drops. */
const pointerThenClosestCorners: CollisionDetection = (args) => {
  const pointerHits = pointerWithin(args);
  if (pointerHits.length > 0) return pointerHits;
  return closestCorners(args);
};

function TaskDragOverlayPreview({ task }: { task: Task | null }) {
  return (
    <div className="flex items-center gap-2 min-w-[280px] max-w-[480px] rounded-md border border-[#ececeb] bg-white py-2.5 pl-1 pr-3 shadow-lg cursor-grabbing">
      <GripVertical className="w-4 h-4 shrink-0 text-[#a5a4a2]" />
      <span className="text-sm font-medium text-[#37352f] truncate">
        {task?.name?.trim() ? task.name : 'Untitled'}
      </span>
    </div>
  );
}
...
export function TodoApp({
  ...
  initialData: TodoDataPayload;
  user: TodoAppUser;
}) {
  const router = useRouter();
  const [tasks, setTasks] = useState<Task[]>(initialData.tasks);
  const [statusOptions, setStatusOptions] = useState(initialData.statusOptions);
  const [priorityOptions, setPriorityOptions] = useState(initialData.priorityOptions);
  const [projectOptions, setProjectOptions] = useState(initialData.projectOptions);
  ...
  const tasksRef = useRef<Task[]>(initialData.tasks);

  const dragEnabled = sortKey === 'none' && filterQuery.trim() === '';
  ...
  useEffect(() => {
    ...
    setStatusOptions(initialData.statusOptions);
    setPriorityOptions(initialData.priorityOptions);
    setProjectOptions(initialData.projectOptions);
  }, [initialData]);

  tasksRef.current = tasks;

  const defaultStatusId =
    statusOptions.find((s) => s.label === 'Not started')?.id ??
    statusOptions[0]?.id ??
    initialData.defaultStatusId;
  const defaultPriorityId =
    priorityOptions.find((p) => p.label === 'Medium')?.id ??
    priorityOptions[0]?.id ??
    initialData.defaultPriorityId;
  ...
  const handleModalUpdate = async (id: string, updates: Partial<Task>) => {
  ...
  const addCreatingTask = async () => {
    if (!creatingTask) return;
    await createTaskAction(creatingTask);
    setCreatingTask(null);
    router.refresh();
  };
  ...
  const deleteTask = async (id: string) => {
    ...
    setEditingTask((p) => (p?.id === id ? null : p));
    setCreatingTask((p) => (p?.id === id ? null : p));
    try {
  ...
  const addStatusOption = async (label: string) => {
    const id = await addStatusOptionAction(label);
    setStatusOptions((prev) => [
      ...prev,
      { id, label, color: 'gray', type: 'not-started' },
    ]);
    router.refresh();
    return id;
  };

  const addPriorityOption = async (label: string) => {
    const id = await addPriorityOptionAction(label);
    setPriorityOptions((prev) => [...prev, { id, label, color: 'gray' }]);
    router.refresh();
    return id;
  };

  const addProjectOption = async (label: string) => {
    const id = await addProjectOptionAction(label);
    setProjectOptions((prev) => [...prev, { id, label, color: 'gray' }]);
    router.refresh();
    return id;
  };
  ...
  const applyList = (list: Task[]) => {
    ...
    if (sortKey === 'name') {
      ...
      t = [...t].sort((a, b) => a.sortIndex - b.sortIndex || a.id.localeCompare(b.id));
    }
    return t;
  };

  const onDragStart = (event: DragStartEvent) => {
    dragSnapshot.current = tasksRef.current.map((t) => ({ ...t }));
  ...
  const onDragEnd = async (event: DragEndEvent) => {
    ...
    const activeTask = snap.find((t) => t.id === activeId);
    if (!activeTask) {
      setTasks(snap);
      return;
    }
    ...
    const overGroupFromDrop = groupFromDropId(overId);
    const overTask = snap.find((t) => t.id === overId);
    const activeGroup = activeTask.group;

    let overGroup: Task['group'];
    let overIndex: number;

    if (overGroupFromDrop) {
      overGroup = overGroupFromDrop;
      overIndex = 0;
    } else if (overTask) {
      overGroup = overTask.group;
      const list = idsInGroup(snap, overGroup);
      overIndex = list.indexOf(overId);
      if (overIndex < 0) {
        setTasks(snap);
        return;
      }
    ...
    const byGroup: Record<Task['group'], string[]> = {
      Today: idsInGroup(snap, 'Today'),
      'This Week': idsInGroup(snap, 'This Week'),
      Later: idsInGroup(snap, 'Later'),
    };

    if (activeGroup === overGroup) {
      const list = [...byGroup[activeGroup]];
      const oldIndex = list.indexOf(activeId);
      if (oldIndex < 0) {
        setTasks(snap);
        return;
      }
      const newIndex = overGroupFromDrop ? 0 : overIndex;
      const newList = arrayMove(list, oldIndex, newIndex);
      byGroup[activeGroup] = newList;
    } else {
      const fromList = [...byGroup[activeGroup]].filter((id) => id !== activeId);
      const toList = [...byGroup[overGroup]].filter((id) => id !== activeId);
      if (overGroupFromDrop) {
        toList.splice(0, 0, activeId);
      } else {
        toList.splice(overIndex, 0, activeId);
      }
      byGroup[activeGroup] = fromList;
      byGroup[overGroup] = toList;
    }

    const nextTasks = rebuildTasksFromGroups(snap, byGroup);
    setTasks(nextTasks);

    const payload = TASK_GROUP_ORDER.flatMap((g) =>
      byGroup[g].map((id, sortIndex) => ({ id, group: g, sortIndex }))
    );

    try {
      await reorderTasksAction(payload);
      router.refresh();
    } catch {
      setTasks(snap);
    }
  ...
  const weekIds = useMemo(() => thisWeekTasks.map((t) => t.id), [thisWeekTasks]);
  const laterIds = useMemo(() => laterTasks.map((t) => t.id), [laterTasks]);
    ...
    <div className="flex h-screen bg-white text-[#37352f] overflow-hidden selection:bg-[rgba(35,131,226,0.14)]">
        ...
        <div className="flex-1 overflow-y-auto py-2">
          ...
          <div className="mt-6">
            <SidebarGroup title="Favorites" defaultExpanded>
              ...
              <SidebarItem icon={<MapIcon className="w-4 h-4 text-[#a5a4a2]" />} label="Project Roadmap" />
        ...
        <div className="flex-1 overflow-y-auto overflow-x-hidden relative">
          <div className="max-w-[1240px] mx-auto px-12 md:px-24 py-12 pb-32">
                  ...
                  {sortOpen && (
                    ...
                    >
                      {(['none', 'name', 'due'] as const).map((k) => (
                        ...
                        >
                          {k === 'none' ? 'Default (manual order)' : k === 'name' ? 'Name' : 'Due date'}
                        </button>
                      ))}
                      {sortKey !== 'none' && (
                        <button
            ...
            <div className="w-full">
              <DndContext
                sensors={sensors}
                collisionDetection={pointerThenClosestCorners}
              ...
              >
                <TaskGroup title="Today">
                  ...
                  <SortableContext items={todayIds} strategy={verticalListSortingStrategy}>
                    ...
                    {todayTasks.length === 0 && dragEnabled ? (
                      <GroupEmptyDropTarget id={DROP_ID.Today} />
                    ) : null}
                  </SortableContext>
                  <div
                    role="button"
                ...
                <div className="mt-8">
                  <TaskGroup title="This Week">
                    ...
                    <SortableContext items={weekIds} strategy={verticalListSortingStrategy}>
                      ...
                      {thisWeekTasks.length === 0 && dragEnabled ? (
                        <GroupEmptyDropTarget id={DROP_ID['This Week']} />
                      ) : null}
                    </SortableContext>
                    <div
                      role="button"
                ...
                <div className="mt-8">
                  <TaskGroup title="Later">
                    <TableHeader />
                    <SortableContext items={laterIds} strategy={verticalListSortingStrategy}>
                      {laterTasks.map((task) => (
                        <TaskTableRow
                          key={task.id}
                          task={task}
                      ...
                      {laterTasks.length === 0 && dragEnabled ? (
                        <GroupEmptyDropTarget id={DROP_ID.Later} />
                      ) : null}
                    </SortableContext>
                    <div
                      role="button"
                ...
                <DragOverlay dropAnimation={null}>
                  {activeDragId ? (
                    <TaskDragOverlayPreview
                      task={tasks.find((t) => t.id === activeDragId) ?? null}
                    />
                  ) : null}
                </DragOverlay>
              </DndContext>
      ...
      {(editingTask || creatingTask) && (
        <TaskModal
          ...
          priorityOptions={priorityOptions}
          projectOptions={projectOptions}
          onCreateStatusOption={addStatusOption}
          onCreatePriorityOption={addPriorityOption}
          onCreateProjectOption={addProjectOption}
        />
      )}
    </div>
  );
...
function TableHeader() {
  return (
    <div className="flex items-center text-sm text-[#787774] font-medium py-2 border-b border-[#ececeb]">
      <div className="w-7 shrink-0 pl-1" aria-hidden />
